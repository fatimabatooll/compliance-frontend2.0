...
const getQuestionType = (question: DimensionQuestion) => {
  ...
  if (type === "binary" || type === "checkbox" || type === "list" || type === "text") {
    return type
  }
  return "text"
}
...
export default function QuestionnairePage({
  ...
  const totalAnswered = useMemo(
    () =>
      Object.values(scoresByDimension).reduce((sum, record) => {
        return sum + Object.values(record.responses).filter((value) => hasAnswer(value)).length
      }, 0),
    [scoresByDimension]
  )
  ...
  const canSubmitAssessment =
    isCurrentDimComplete && effectiveAnsweredCount >= MIN_REQUIRED_ANSWERS_TO_SUBMIT
  ...
  const isCurrentDimensionDirty = useMemo(() => {
    ...
    const toComparable = (source: Record<string, AnswerValue>) => {
      return currentQuestions.reduce<Record<string, string>>((acc, question) => {
      ...
      try {
        ...
        const scores =
          company?.status === "pending"
            ? []
            : await questionnaireService.viewResponses(id, READINESS_INDEX, token)
    ...
    if (type === "checkbox") {
      if (Array.isArray(value) && question.checkboxes?.length) {
        return value.reduce((sum, selected) => {
          const match = question.checkboxes?.find((item) => item.option === selected)
  ...
  const refreshScores = async () => {
    if (!token) return
    const scores = await questionnaireService.viewResponses(id, READINESS_INDEX, token)
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      <div className="fixed top-0 left-0 right-0 z-40 bg-background/80 backdrop-blur-xl border-b border-border/50 pt-24 mt-2">
                  ...
                  <div className="mb-6">
                    <div className="mb-4">
                      <span className="text-xs font-medium text-primary uppercase tracking-wider">
                        {currentMeta.domainTitle} • Dimension {currentDimIndex + 1} of{" "}
                        {dimensions.length}
                      </span>
                    ...
                    <div className="glass rounded-2xl p-4 mb-6">
                      <div className="flex items-center justify-between mb-3">
                        <span className="text-sm font-medium text-foreground">
                          {dimAnsweredCount} of {currentQuestions.length} answered
                        </span>
                        <span
                          className={cn(
                      ...
                      <div className="w-full h-2 bg-secondary rounded-full overflow-hidden">
                        <div
                          ...
                          style={{
                            width: `${
                              (dimAnsweredCount / Math.max(currentQuestions.length, 1)) *
                              100
                            }%`,
                          }}
                        />
                    ...
                    {currentDimIndex === dimensions.length - 1 &&
                      effectiveAnsweredCount < MIN_REQUIRED_ANSWERS_TO_SUBMIT && (
                        <p className="mt-3 text-xs text-muted-foreground">
                          Complete at least {MIN_REQUIRED_ANSWERS_TO_SUBMIT} answered
                          questions before submitting ({effectiveAnsweredCount}/
