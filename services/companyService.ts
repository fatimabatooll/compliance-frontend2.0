...
const deriveScore = (
  ...
  const fromOverall = raw.overAllScore?.find(
    (item) => item.index === readinessIndexType,
  );
  const fallbackOverall = raw.overAllScore?.find(
    (item) => item.index === "genai",
  );
  ...
  if (
    typeof raw.evaluationDetails?.[readinessIndexType]?.overallScore ===
    "number"
  ) {
...
const deriveProgress = (
  ...
  return safeNumber(current?.progress, -1) >= 0
...
class CompanyService {
  ...
  async getCompaniesByConsultantId(
    ...
    const response = await apiService.get<
      ApiResponse<ApiConsultantPayload> | ApiConsultantPayload
    >(`/consultant/${consultantId}`, token);
  ...
  async getCompanyById(
    ...
    const response = await apiService.get<
      ApiResponse<ApiCompanyPayload> | ApiCompanyPayload
